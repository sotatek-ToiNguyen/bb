			...
			class Kirki_Installer_Section extends WP_Customize_Section {
				...
				protected function install_button() {
					...
					<p style="text-align:left;margin-top:0;">
						<?php esc_attr_e( 'Please install the Kirki plugin to take full advantage of this theme\'s customizer capabilities', 'dici' ); ?>
					</p>
					<p style="text-align:left;">
						<?php esc_html_e( 'You can find Kirki plugin in the WordPress plugin directory', 'dici' ); ?>
					</p>
					<?php
				}
				...
				protected function activate_button() {
					?>
					<p style="text-align:left;margin-top:0;">
						<?php esc_attr_e( 'You have installed Kirki. Activate it to take advantage of this theme\'s features in the customizer.', 'dici' ); ?>
					</p>
					<a class="activate-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="<?php esc_attr_e( 'Activate Kirki Toolkit now', 'dici' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Activate Now', 'dici' ); ?>
					</a>
					<?php
				}
				...
				protected function dismiss_button() {
					...
					$show_confirm = true;
					?>
					<a class="kirki-installer-dismiss button-secondary button" data-slug="kirki" href="#" aria-label="<?php esc_attr_e( 'Don\'t show this again', 'dici' ); ?>" data-name="<?php esc_attr_e( 'Dismiss', 'dici' ); ?>">
						<?php esc_attr_e( 'Don\'t show this again', 'dici' ); ?>
					</a>
					...
					<script>
                        jQuery( document ).ready( function() {
                            jQuery( '.kirki-installer-dismiss' ).on( 'click', function( event ) {
								...
								<?php if ( $show_confirm ) : ?>
                                if ( ! confirm( '<?php esc_attr_e( 'Are you sure? Dismissing this message will hide the installation recommendation and you will have to manually install and activate the plugin in the future.', 'dici' ); ?>' ) ) {
                                    return;
                                }
								<?php endif; ?>
